...
# npm uses a configurable registry. Default is npmmirror (fast in China; TUNA's
# npm mirror does not carry the scoped @deepseek-ai packages). CI overrides it
# with the official registry (registry.npmjs.org) because GitHub-hosted runners
# reach npmmirror slowly and time out mid-install. Override with
# --build-arg NPM_REGISTRY=<url>.
...
ARG PNPM_VERSION=10
RUN set -eux; \
    # NodeSource's apt nodejs places `node` at /usr/bin, so npm infers its
    # global prefix as /usr (not /usr/local) and installs global packages to
    # /usr/lib/node_modules. Pin the prefix explicitly so global packages land
    # where Dockerfile/entrypoint expect them (/usr/local/lib/node_modules).
    npm_config_registry="$NPM_REGISTRY"; \
    npm_config_prefix=/usr/local; \
    export npm_config_registry npm_config_prefix; \
    ...
    echo "installed global packages: $(ls /usr/local/lib/node_modules/@deepseek-ai/ 2>/dev/null | tr '\n' ' ')"; \
    test -f /usr/local/lib/node_modules/@deepseek-ai/dsh/lib/bin.js || { \
        ...
        echo "--- actual npm root: $(npm root -g) ---" >&2; \
        ls -la "$(npm root -g)/@deepseek-ai/" 2>/dev/null >&2 || true; \
        exit 1; \
    }; \
